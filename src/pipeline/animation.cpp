...
void Skeleton::computeFinalBoneMatrices( std::vector<Matrix44>& bone_matrices, GFX::Mesh* mesh )
{
	assert(mesh);
...
void Skeleton::renderSkeleton(Camera* camera, Matrix44 model, Vector4f color, bool render_points)
{
	GFX::Mesh m;
	...
	if (render_points)
	{
		shader->setUniform("u_color", color * 2.0f);
		glPointSize(10);
		m.render(GL_POINTS);
		glPointSize(1);
	}
...
bool Animation::load(const char* filename)
{
	...
	std::cout << " + Animation loading: " << TermColor::YELLOW << filename << TermColor::DEFAULT << " ... ";
	long time = getTime();
